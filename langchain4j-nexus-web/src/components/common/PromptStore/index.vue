...
const modalMode = ref('')
// 这个是为了后期的修改Prompt内容考虑，因为要针对无uuid的list进行修改，且考虑到不能出现标题和内容的冲突，所以就需要一个临时item来记录一�?const tempModifiedItem = ref<any>({})
// 添加修改导入都使用一个Modal, 临时修改内容占用tempPromptKey,切换状态前先将内容都清�?const changeShowModal = (mode: 'add' | 'modify' | 'local_import', selected = { act: '', prompt: '', id: 0 }) => {
  if (mode === 'add') {
    tmpPromptKey.value = ''
...
const importPromptTemplate = async (from = 'online') => {
  try {
    ...
    console.error(error)
    message.error('JSON 格式错误，请检�?JSON 格式')
  }
  if (from === 'local')
    showModal.value = !showModal.value
}
