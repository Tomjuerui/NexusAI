...
async function startRecording() {
  ...
  try {
    ...
    if (exception.message.includes('Permission denied'))
      ...
      errorMsg.value = '没有找到录音设备，请检查设备连�?
...
<template>
  <div class="flex flex-col items-center space-y-3">
    ...
    />
    <AudioWaveIcon v-if="recording" class="cursor-pointer" placeholder="对话�? @click="toggleRecording" />
    <div v-if="!recording" class="text-sm text-gray-500">
      点击开始对�?    </div>
    <div v-if="recording" class="text-sm text-gray-500">
      对话�?{{ recordingDuration / 1000 }}�?，点击图标结�?    </div>
