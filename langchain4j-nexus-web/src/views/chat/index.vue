...
// 打开知识库引�?async function handleEmbeddingRefClick(qaRecordUuid: string) {
...
const fetchChatAPIOnce = async (regenerateQuestionUuid: string, childAudioPlayState: AudioPlayState) => {
  ...
  api.sseProcess({
    ...
    thinkingDataReceived: (chunk) => {
      ...
      // 推理阶段无需显示状�?      question.state = new Map<string, string>()
    },
    messageReceived: (chunk) => {
      ...
      if (ttsPartText && appStore.audioSynthesizerSide === AUDIO_SYNTHESIZER_SIDE.client && answerContentType === CHAT_MESSAGE_CONTENT_TYPE.audio && conv.isAutoplayAnswer) {
        // settimeout是防止执行太快导�?AudioMessage 中的 watch 没有触发
        setTimeout(() => {
    ...
    audioDataReceived(audioFrame) {
      // AudioMessage 监听pcmPart的变化并决定要不要自动播�?      if (appStore.audioSynthesizerSide !== AUDIO_SYNTHESIZER_SIDE.client && audioFrame)
    ...
    doneCallback: (chunk) => {
      ...
      if (chunk.includes('[META]')) {
        ...
        // 保留临时answer的uuid以方便自动选中最新答�?        updateMessageSomeFields(convUuid, answer.uuid, { ...metaData.answer, uuid: answer.uuid, loading: false })
  ...
  <div class="chat-box flex flex-col w-full h-full">
    ...
    <NModal v-model:show="showRefEmbeddingModal" style="max-width: 80%;" preset="card" title="引用资料">
      <div v-show="knowledgeEmbeddingRef.length === 0" class="flex items-center justify-center h-64">
        <span v-show="!loaddingEmbeddingRef">无数�?/span>
    ...
    <NModal v-model:show="showMemoryModal" style="max-width: 80%;" preset="card" title="命中的记�?>
      <div v-show="memoryEmbeddings.length === 0" class="flex items-center justify-center h-64">
        <span v-show="!loaddingMemory">无数�?/span>
