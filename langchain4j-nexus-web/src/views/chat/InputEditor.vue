...
const fetchChatAPIOnce = async (message: string, userAudioUuid: string, userAudioDuration: number) => {
  ...
  api.sseProcess({
    ...
    thinkingDataReceived: (chunk) => {
      // 处理思考数�?      console.log('Thinking data received:', chunk)
      ...
      // 推理阶段无需显示状�?      chattingMsg.value.state = new Map<string, string>()
    },
    messageReceived: (chunk) => {
      ...
      // 回复阶段无需显示状�?      chattingMsg.value.state = new Map<string, string>()
    },
