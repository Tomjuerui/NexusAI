package com.moyz.nexus.common.memory.longterm;
...
import com.moyz.nexus.common.memory.vo.ActionMemories;
import com.moyz.nexus.common.memory.vo.ExtractedFact;
import com.moyz.nexus.common.rag.EmbeddingRagContext;
...
import com.moyz.nexus.common.vo.ChatModelRequestParams;
...
import static com.moyz.nexus.common.cosntant.NexusConstant.MetadataKey.CONVERSATION_MSG_ID;
...
public class LongTermMemoryService {
    ...
    public void asyncAdd(Long convId, String modelPlatform, String modelName, String userMessage, String assistantMessage) {
        ...
        String factResponse = NexusStringUtil.removeCodeBlock(response.aiMessage().text());
        if (StringUtils.isBlank(factResponse)) {
            log.warn("无法针对本次内容整理出事实性信�?);
            return;
        }
        ...
        for (String fact : facts) {
            ...
            String analyzedMsg = NexusStringUtil.removeCodeBlock(resp);
            ...
            for (ActionMemories.ActionMemory actionMemory : actionMemories.getMemory()) {
                ...
                } else if (NexusConstant.MemoryEvent.DELETE.equalsIgnoreCase(actionMemory.getEvent())) {
                    String embeddingId = tmpIdToEmbeddingId.get(Integer.parseInt(actionMemory.getId()));
                    convMemoryEmbeddingStore.remove(embeddingId);
                } else if (NexusConstant.MemoryEvent.UPDATE.equalsIgnoreCase(actionMemory.getEvent())) {
                ...
                } else if (NexusConstant.MemoryEvent.ADD.equalsIgnoreCase(actionMemory.getEvent())) {
                    Metadata metadata = new Metadata(Map.of(CONVERSATION_ID, convId));
                    Document document = new DefaultDocument(actionMemory.getText(), metadata);
                    EmbeddingRagContext.get(NexusConstant.RetrieveContentFrom.CONV_MEMORY).ingest(document, 20, null, null);
                }
            }
        }
    }
