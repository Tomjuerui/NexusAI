...
public class GraphExtractPrompt {
    public static final String GRAPH_EXTRACTION_PROMPT = """
            ...
            Text: {input_text}
            ######################
            Output:""".replace("{tuple_delimiter}", NexusConstant.GRAPH_TUPLE_DELIMITER)
            .replace("{entity_types}", String.join(",", NexusConstant.GRAPH_ENTITY_EXTRACTION_ENTITY_TYPES))
            .replace("{completion_delimiter}", NexusConstant.GRAPH_COMPLETION_DELIMITER)
            .replace("{record_delimiter}", NexusConstant.GRAPH_RECORD_DELIMITER);
    ...
    public static final String GRAPH_EXTRACTION_PROMPT_CN = """
            ...
            - entity_description：实体的属性和活动的全面描�?
            将每个实体格式化�?("entity"{tuple_delimiter}<entity_name>{tuple_delimiter}<entity_type>{tuple_delimiter}<entity_description>)
                        
            2. 从步�?中识别出的实体中，识别出所有明确相关的 (source_entity, target_entity) 对�?
            对于每对相关的实体，提取以下信息�?
            - source_entity：在步骤1中识别的源实体的名称
            - target_entity：在步骤1中识别的目标实体的名�?
            - relationship_description：解释你认为源实体和目标实体之间相关的原�?
            - relationship_strength：一个表示源实体和目标实体之间关系强度的数字分数
            将每个关系格式化�?("relationship"{tuple_delimiter}<source_entity>{tuple_delimiter}<target_entity>{tuple_delimiter}<relationship_description>{tuple_delimiter}<relationship_strength>)
                        
            3. 以英文返回输出，作为所有在步骤1和步�?中识别的实体和关系的列表。使�?**{record_delimiter}** 作为列表分隔符�?
            ...
            ######################
            输出:
            """.replace("{tuple_delimiter}", NexusConstant.GRAPH_TUPLE_DELIMITER)
            .replace("{entity_types}", String.join(",", NexusConstant.GRAPH_ENTITY_EXTRACTION_ENTITY_TYPES))
            .replace("{completion_delimiter}", NexusConstant.GRAPH_COMPLETION_DELIMITER)
            .replace("{record_delimiter}", NexusConstant.GRAPH_RECORD_DELIMITER);
