...
import com.moyz.nexus.common.entity.WorkflowEdge;
import com.moyz.nexus.common.enums.ErrorEnum;
import com.moyz.nexus.common.exception.BaseException;
import com.moyz.nexus.common.mapper.WorkflowEdgeMapper;
        ...
        for (WfEdgeReq edge : edges) {
            ...
            if (null != old) {
                if (!old.getWorkflowId().equals(edge.getWorkflowId())) {
                    log.error("该边不属于指定的工作�?保存失败,workflowId:{},old workflowId:{},new workflowId:{}", workflowId, old.getWorkflowId(), edge.getWorkflowId());
                    throw new BaseException(ErrorEnum.A_PARAMS_ERROR);
                }
                log.info("更新�?id:{},uuid:{},source:{},sourceHandle:{},target:{}",
                        edge.getId(), edge.getUuid(), edge.getSourceNodeUuid(), edge.getSourceHandle(), edge.getTargetNodeUuid());
            } else {
                newOne.setId(null);
                log.info("新增�?uuid:{},source:{},sourceHandle:{},target:{}",
    ...
    public void deleteEdges(Long workflowId, List<String> uuids) {
        ...
        for (String uuid : uuids) {
            ...
            if (null != old && !old.getWorkflowId().equals(workflowId)) {
                log.error("该边不属于指定的工作�?删除失败,workflowId:{},node workflowId:{}", workflowId, workflowId);
                throw new BaseException(ErrorEnum.A_PARAMS_ERROR);
            }
