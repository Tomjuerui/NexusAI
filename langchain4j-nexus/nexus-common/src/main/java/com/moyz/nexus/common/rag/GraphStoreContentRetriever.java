...
public class GraphStoreContentRetriever implements ContentRetriever {
    ...
    public List<Content> retrieve(Query query) {
        ...
        String[] records = response.split(NexusConstant.GRAPH_RECORD_DELIMITER);
        for (String record : records) {
            String newRecord = record.replaceAll("^\\(|\\)$", "");
            String[] recordAttributes = newRecord.split(NexusConstant.GRAPH_TUPLE_DELIMITER);
            if (recordAttributes.length >= 4 && (recordAttributes[0].contains("\"entity\"") || recordAttributes[0].contains("\"实体\""))) {
                entities.add(NexusStringUtil.clearStr(recordAttributes[1].toUpperCase()));
                ...
                entities.add(NexusStringUtil.clearStr(sourceName));
                entities.add(NexusStringUtil.clearStr(targetName));
            }
        }
        ...
        entities = entities.stream().map(NexusStringUtil::removeSpecialChar).filter(StringUtils::isNotBlank).collect(Collectors.toSet());
        if (entities.isEmpty()) {
            log.info("从用户查询中没有解析出实�?);
            return Collections.emptyList();
        }
