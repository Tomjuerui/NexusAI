...
import com.moyz.nexus.common.util.GraphStoreUtil;
...
import static com.moyz.nexus.common.enums.ErrorEnum.B_DB_ERROR;
    ...
    public boolean addVertexes(List<GraphVertex> vertexes) {
        ...
        try (Connection connection = setupConnection()) {
            for (GraphVertex vertex : vertexes) {
                vertex.setName(NexusStringUtil.tail(vertex.getName(), 20));
