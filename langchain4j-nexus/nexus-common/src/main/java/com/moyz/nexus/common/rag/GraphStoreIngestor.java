...
import com.moyz.nexus.common.util.NexusStringUtil;
...
import static com.moyz.nexus.common.cosntant.NexusConstant.MAX_METADATA_VALUE_LENGTH;
...
public class GraphStoreIngestor {
    ...
    public void ingest(List<Document> documents) {
        ...
        for (Triple<TextSegment, String, String> triple : segmentIdToAiResponse) {
            ...
            String[] rows = StringUtils.split(response, NexusConstant.GRAPH_RECORD_DELIMITER);
            for (String row : rows) {
                String graphRow = row;
                graphRow = graphRow.replaceAll("^\\(|\\)$", "");
                String[] recordAttributes = StringUtils.split(graphRow, NexusConstant.GRAPH_TUPLE_DELIMITER);
                if (recordAttributes.length >= 4 && (recordAttributes[0].contains("\"entity\"") || recordAttributes[0].contains("\"实体\""))) {
                    String entityName = NexusStringUtil.clearStr(recordAttributes[1].toUpperCase());
                    String entityType = NexusStringUtil.clearStr(recordAttributes[2].toUpperCase()).replaceAll("[^a-zA-Z0-9\\s\\u4E00-\\u9FA5]+", "").replace(" ", "");
                    String entityDescription = NexusStringUtil.clearStr(recordAttributes[3]);
                    ...
                    String sourceName = NexusStringUtil.clearStr(recordAttributes[1].toUpperCase());
                    String targetName = NexusStringUtil.clearStr(recordAttributes[2].toUpperCase());
                    String edgeDescription = NexusStringUtil.clearStr(recordAttributes[3]);
                    log.info("Relationship sourceName:{},targetName:{},edgeDescription:{}", sourceName, targetName, edgeDescription);
                    String chunkId = NexusStringUtil.clearStr(textSegmentId);
    ...
    /**
     * metadata记录的值为Map，如：kb_uuid=>123,kb_item_uuid=>22222,3333，其中类�?3333 的值是追加的，超过最大限度时丢弃最早的数据
     * TODO 重构以记录所有追加的�?
     *
