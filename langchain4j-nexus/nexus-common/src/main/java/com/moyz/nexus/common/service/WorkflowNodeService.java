...
import com.moyz.nexus.common.entity.Workflow;
...
import com.moyz.nexus.common.mapper.WorkflowNodeMapper;
...
import com.moyz.nexus.common.workflow.def.WfNodeIOText;
import com.moyz.nexus.common.workflow.node.mailsender.MailSendNodeConfig;
...
public class WorkflowNodeService extends ServiceImpl<WorkflowNodeMapper, WorkflowNode> {
        ...
        for (WfNodeDto node : nodes) {
            ...
            if (null != old) {
                if (!old.getWorkflowId().equals(node.getWorkflowId())) {
                    log.error("节点不属于指定的工作�?保存失败,workflowId:{},old workflowId:{},new workflowId:{},node uuid:{},title:{}",
    ...
    private void checkAndEncrypt(WorkflowNode workflowNode) {
        WorkflowComponent component = workflowComponentService.getAllEnable()
                ...
                .orElse(null);
        if (null == component) {
            log.error("节点不存�?uuid:{},title:{}", workflowNode.getUuid(), workflowNode.getTitle());
            throw new BaseException(ErrorEnum.A_PARAMS_ERROR);
        }
    ...
    public void deleteNodes(Long workflowId, List<String> uuids) {
        ...
        for (String uuid : uuids) {
            ...
            if (!old.getWorkflowId().equals(workflowId)) {
                log.error("节点不属于指定的工作�?删除失败,workflowId:{},node workflowId:{}", workflowId, workflowId);
                throw new BaseException(ErrorEnum.A_PARAMS_ERROR);
            }
            if (workflowComponentService.getStartComponent().getId().equals(old.getWorkflowComponentId())) {
                log.warn("开始节点不能删�?uuid:{}", old.getUuid());
                continue;
            }
